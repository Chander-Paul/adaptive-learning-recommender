...
class MultiViewLearningPathClusterer:
    ...
    def __init__(self, data_path='data/training_files/student_performance.csv',
        ...
        self.style_interpretations = None
        self.performance_interpretations = None
        self.affective_interpretations = None
    ...
    def cluster_learning_styles(self, n_clusters=None):
        ...
        cluster_profile_df = self.style_clusterer.analyze_clusters()
        ...
        self.style_interpretations = self.interpret_learning_styles(cluster_profile_df)

        return self.style_labels, self.style_interpretations
    ...
    def cluster_performance(self, n_clusters=None):
        ...
        cluster_profile_df = self.performance_clusterer.analyze_clusters()
        ...
        self.performance_interpretations = self.interpret_performance(cluster_profile_df)
        self.performance_labels = self.performance_clusterer.cluster_labels
        self.df['Performance_Cluster'] = self.performance_labels # Save the trained model for future use
        return self.performance_labels, self.performance_interpretations
    ...
    def cluster_affective(self, n_clusters=3):
        ...
        cluster_profile_df = self.affective_clusterer.analyze_clusters()
        ...
        affective_interpretations = self.interpret_affective(cluster_profile_df)
        self.affective_interpretations = affective_interpretations
        return self.affective_labels, affective_interpretations

    def interpret_learning_styles(self, cluster_profile_df):
    ...
    def interpret_performance(self, cluster_profile_df):
    ...
    def interpret_affective(self, cluster_profile_df):
    ...
    def visualize_cluster_view(self, view_name, data, labels, n_clusters):
        """
    ...
    def load_interpretations(self, dir_path='models/learner'):
        """Load saved cluster interpretations."""
        dirpath = os.path.join(dir_path, 'clusters/')
        with open(os.path.join(dirpath, 'multiview_interpretations.json'), 'r') as f:
            interpretations = json.load(f)
        
        self.style_interpretations = interpretations.get('style', {})
        self.performance_interpretations = interpretations.get('performance', {})
        self.affective_interpretations = interpretations.get('affective', {})
        
        
    
    def save_results(self, cluster_interpretations, dirpth = "models/learner/clusters/"):
        ...
        output_df.to_csv(os.path.join(dirpth, 'multiview_student_profiles.csv'), index=False)
        print(f"Saved: {os.path.join(dirpth, 'multiview_student_profiles.csv')}")
        
        # Save results
        print(cluster_interpretations)
        with open(os.path.join(dirpth, 'multiview_results.json'), 'w') as f:
            json.dump(cluster_interpretations, f, indent=2)
        print(f"Saved: {os.path.join(dirpth, 'multiview_results.json')}")


        ## save interpretations


        with open(os.path.join(dirpth, 'multiview_interpretations.json'), 'w') as f:
            json.dump({
                'style': self.style_interpretations,
                'performance': self.performance_interpretations,
                'affective': self.affective_interpretations
            }, f, indent=2)
        print(f"✓ Saved: {os.path.join(dirpth, 'multiview_interpretations.json')}")
    ...
    def run_full_multiview_analysis(self):
        ...
        results = self.create_composite_profiles(
        ...
        print(f"\nComposite Profiles: {len(results)}")
    ...
    def predict_student_profile(self, student_features):
        ...
        # Finally, it would combine those labels into a composite profile and return results.
        ...
        style_interpretations = self.style_interpretations.get(str(style_cluster), {})
        perf_interpretations = self.performance_interpretations.get(str(perf_cluster), {})
        affective_interpretations = self.affective_interpretations.get(str(engage_cluster), {})

        composite_profile = [style_cluster, perf_cluster, engage_cluster]
        composite_profile_interpretations = [style_interpretations, perf_interpretations, affective_interpretations]
        profile = list(zip(composite_profile, composite_profile_interpretations))
        
        return profile
    
    def create_composite_profiles(self, style_interp, perf_interp, eng_interp):
        """Combine cluster interpretations into composite profiles and generate results."""
        composite_profiles = {}
        
        for style_id, style_info in style_interp.items():
            for perf_id, perf_info in perf_interp.items():
                for eng_id, eng_info in eng_interp.items():
                    profile_key = f"Style{style_id}_Perf{perf_id}_Eng{eng_id}"
                    composite_profiles[profile_key] = {
                        'LearningStyle': style_info['Style'],
                        'Performance': perf_info['Level'],
                        'Affective': eng_info['Level'],
                        'Count': 0  # Will be updated later
                    }
        
        # Count how many students fall into each composite profile
        for _, row in self.df.iterrows():
            profile_key = f"Style{row['LearningStyle_Cluster']}_Perf{row['Performance_Cluster']}_Eng{row['Affective_Cluster']}"
            if profile_key in composite_profiles:
                composite_profiles[profile_key]['Count'] += 1
        
        # Calculate percentages
        total_students = len(self.df)
        for key in composite_profiles:
            count = composite_profiles[key]['Count']
            composite_profiles[key]['Percentage'] = f"{(count / total_students * 100):.1f}%"
        
        return composite_profiles   
    ...
    def save_model(self, dir_path=None):
        ...
        if dir_path is None:
            dir_path = 'models/learner'
        os.makedirs(dir_path, exist_ok=True)
        self.style_clusterer.save_model(os.path.join(dir_path, 'learning_style_clusterer.pkl'))
        self.performance_clusterer.save_model(os.path.join(dir_path, 'performance_clusterer.pkl'))
        self.affective_clusterer.save_model(os.path.join(dir_path, 'affective_clusterer.pkl'))
    ...
    def load_model(self, path = 'models/learner'):
        """Load the trained clusterers."""
        self.style_clusterer.load_model(os.path.join(path, 'learning_style_clusterer.pkl'))
        self.performance_clusterer.load_model(os.path.join(path, 'performance_clusterer.pkl'))
        self.affective_clusterer.load_model(os.path.join(path, 'affective_clusterer.pkl'))
        ...
        #self.style_interpretations = self.interpret_learning_styles(self.style_clusterer.analyze_clusters())
        #self.performance_interpretations = self.interpret_performance(self.performance_clusterer.analyze_clusters())
        #self.affective_interpretations = self.interpret_affective(self.affective_clusterer.analyze_clusters())
        self.load_interpretations(dir_path=path)
...
def main():
    ...
    results = multiview.run_full_multiview_analysis()
    
    # Print sample results
    print("\n" + "="*70)
    print("Example Composite Results")
    print("="*70)
    
    for i, (profile, rec) in enumerate(list(results.items())[:5]):
        print(f"\nProfile {i+1}: {profile}")
        ...
        print(f"  Percentage: {rec['Percentage']}")
